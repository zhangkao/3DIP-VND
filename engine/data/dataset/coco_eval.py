...
class CocoEvaluator(object):
    ...
    def summarize(self):
        for iou_type, coco_eval in self.coco_eval.items():
            ...
            coco_eval.summarize()
            summarize_per_class_ap50(coco_eval)
...
def summarize_per_class_ap50(coco_eval):
    import itertools

    # precision: [TxRxKxAxM]
    # T = #IoU thresholds (usually 10)
    # R = #recall thresholds (usually 101)
    # K = #classes
    # A = #area range
    # M = #max detections
    precisions = coco_eval.eval['precision']  # shape: (T, R, K, A, M)

    iou_thr_index = list(coco_eval.params.iouThrs).index(0.5)  # index for IoU=0.50
    area_index = 0  # 0: all area
    max_det_index = 2  # 2: maxDets=100

    # 获取 category id 到 name 的映射
    id2name = {cat['id']: cat['name'] for cat in coco_eval.cocoGt.cats.values()}
    class_names = [id2name[cid] for cid in coco_eval.params.catIds]

    print("\nPer-category AP50:")
    for class_idx, class_id in enumerate(class_names):
        precision = precisions[iou_thr_index, :, class_idx, area_index, max_det_index]
        precision = precision[precision > -1]
        ap50 = np.mean(precision) if precision.size > 0 else float('nan')
        print(f"Class {class_names[class_idx]:>15}: AP50 = {ap50:.3f}")
